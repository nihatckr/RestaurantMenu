...
| B.13 | (My flag) Any must-have missing? → **Data safety** | seed→DB removed the "re-seed" safety net; DB loss / bad delete = menu gone | Added a **Data safety** section: verified DB backups/PITR (#1), soft-delete/trash, JSON export/import, login break-glass, blob orphan cleanup, Turkish admin UI. Ships **with** the admin. | I raised it; user asked "olmazsa olmaz eklenecek konu var mı" | *(planning)* |
