...
| B.17 | Auth method (magic-link vs single password) | One admin only (owner) | **Single password**, owner-only — `iron-session` (encrypted cookie) + **argon2** hash in env (`ADMIN_PASSWORD_HASH`); no email/multi-user/Auth.js overhead; `/login`+logout, login rate-limit, env break-glass. Auth.js is the upgrade path if users/OAuth are added. Resolves U-admin-1. | `"sadece sahip düzenleyecek, tek şifre ile devam et"` | *(planning)* |
