...
| B.21 | Category CRUD as a **separate panel** vs inline on the existing list | Don't duplicate the list | Reworked into `CategoryNavIsland`: guests get the plain nav; admin gets the **same list** with inline edit/delete + "add" (no second panel). | `"zaten kategori list var … yeniden bir liste ekledin"` | ✅ done |
| B.22 | Admin mode + logout placement | Owner wanted it in the header, not a floating bar | Moved from the fixed bottom-left `AdminBar` to an `AdminSessionControls` chip in the **page header** — the category page's **sticky bar** and the landing/chooser switcher row (session-gated island). | `"admin modu ve çıkış butonu sticky header da olabilir mi"` | ✅ done |
| B.23 | Product CRUD as a separate panel vs inline on the menu | Same "no duplicate list" rule as categories | Inline on the category page: each product card/table row gets `ProductRowControls` (edit/delete), each section an `AddProductButton`. Guest render stays **static** — a `MenuSectionsIsland` gates on session; guests get the prerendered `GuestSections` fallback, an admin gets the same sections augmented. Added `productId` to `MenuItemView` to link a card to its Product. | (follows the category inline decision) | ✅ done |
| B.24 | Show/hide products & categories without deleting | Owner needs a temporary "out of stock"/off-menu toggle | Inline eye/eye-off toggle: products via per-venue `MenuItem.available`, categories via `MenuCategory.visible`. The admin sees **hidden** items too (greyed) so they can re-enable them — a fresh `getVenueMenuAdmin` returns hidden items; the public `getVenueMenu` still filters them out. Selects/mapping centralized (`menuCategorySelect`/`menuItemSelect` + `mapMenu`) so public & admin reads never drift. | (chosen next: "Görünürlük aç/kapa") | ✅ done |
| B.25 | Reordering — drag-drop vs arrows | No new dependency; keyboard/tap friendly | Up/down arrow buttons (edges disabled) on each category (landing manager) and each product (ungrouped category grid). A shared `reorder()` helper **renumbers** the whole list to its new index order in one transaction, so it's robust even if sortOrders were equal. Grouped hard-drink tables keep their tag grouping (no arrows) for now. | (chosen next: "Sıralama") | ✅ done |
| B.26 | Measure prices (Kadeh/Şişe, CL) in the product form | Spirits/wines are measure-priced, not single-price | The product form gets a dynamic list of labelled measures (`priceLabel`/`priceAmount` rows) writing `MenuItemPrice`. Measures **supersede** the single price (item price set null). Update is a full replace (clear + recreate). `productSchema.prices` validates; `getProductsAdmin` returns them so edit prefills. | (chosen next: "Ölçülü fiyat") | ✅ done |
| B.27 | Image storage provider | Deploy target is Vercel; B.12 picked "blob storage" | **Vercel Blob** (`@vercel/blob` + `BLOB_READ_WRITE_TOKEN`). `sharp` re-encodes every upload to a bounded, metadata-stripped WebP (SECURITY.md §2). **Dev/e2e fallback:** when the token is unset, write to `/public/uploads` (gitignored) so the flow is testable without a token. `next.config` allows the blob host + CSP `img-src`. | (chosen: "Vercel Blob") | ✅ done |
| B.28 | Image scope this round | Owner picked "product + logo/favicon (all)" | **Product photos:** `ImageField` (pick/preview/remove) in the product form. **Settings page** (`/[locale]/settings`, admin-only, header link): brand logo (`Business.logo`, new migration) + per-venue wordmarks (`Venue.wordmark`). Brand mark now **data-driven** (`getBrandLogo()` with `BRAND.mark` fallback) across chooser/landing/category. **Favicon** generated from the logo via `/brand-icon` (sharp → 64px PNG, packaged-mark + static-icon fallbacks). | (chosen: "Ürün + logo/favicon (hepsi)") | ✅ done |
| B.29 | Settings page too plain | Owner: make it more user-friendly | Reworked into **cards** (`Card`/`CardHeader`, one per section with a lucide icon + title + hint), a top **section-nav** (anchor chips: Marka · Mekanlar · Yedek · Çöp · İşlemler), a 2-col wordmark grid, and inline **"Kaydedildi ✓"** save feedback. Pure presentation — same actions/e2e. | `"settings sayfası daha user friendly olabilir mi"` | ✅ done |
| B.30 | Product/category form fields unclear | Owner reviewed the forms | Added `Field` **hints**: Tür (what it changes), Tek fiyat (superseded by measures), Ölçülü fiyat (what it's for). The **tag** field now **lists the existing sub-groups by name** (Viski, Rakı…) as a hint + a `<datalist>` autocomplete, so the owner picks an existing group instead of guessing. Kept the category picker (enables moving) and the category `columns` input **as-is** per the owner. Then added **descriptions on every form field** (intro line + per-field hints) across both forms. | `"stün sayısı… formları gözden geçirelim"` + `"formlarda kullanıcıya ne olduğuna dair açıklamalar verilmeli"` | ✅ done |
| B.31 | Settings: emptiable trash + tabbed sections | Owner request | **Empty trash:** `emptyTrash()` permanently deletes all trashed rows (frees FKs, cascades, cleans blobs) behind a confirm modal ("Çöp kutusunu boşalt"). **Tabs:** `SettingsTabs` (client) shows one server-rendered panel at a time — panels stay mounted (`hidden`) so form state survives. e2e for the two trash tests is `describe.serial` (the global purge must not race restore). | `"çöp kutusu boşaltılabilmeli ve sekmeler tab şeklinde açılmalı"` | ✅ done |
| B.32 | Uploaded logo "disappears on refresh" | Root-caused: no `BLOB_READ_WRITE_TOKEN` → local-disk fallback, which is ephemeral on Vercel | Data/serving are correct locally (verified). On **Vercel without a token** the fs is ephemeral → uploads vanish → `uploadImage` now **throws a clear error** (guarded by `process.env.VERCEL`) instead of silently losing files; local dev/e2e keep the disk fallback. `.env.example` marks the token required in prod. | `"marka logosunu sürekli eklemem gerekiyor sayfa yenilendiğinde neden"` | ✅ fixed |
| B.33 | More settings features (owner picked all four) | Owner request | **(1) Şifre değiştirme** — Güvenlik tab: change password after verifying the current one (`setPassword`, bcrypt). **(2) QR kodları** — per-venue QR of the public menu URL (`/admin/qr/[slug]`, qrcode → PNG) with preview + download. **(3) İşletme** — business name + optional footer line (`Business.footerExtra`, shown below the fixed legal notice). **(4) Mekanlar** — full venue CRUD (add/rename/wordmark/reorder/delete; guards last venue; cascades menu only). All audited; each verified by e2e. | `"settings sayfasına başka neler eklenebilir"` → all four | ✅ done |
| B.34 | Bootstrap: a fresh deploy must be usable with only the admin seeded | Owner: "sadece admin bilgisi olacak" | `seed:admin` now also **creates a Business** if none exists (name from `BUSINESS_NAME`) — without it the panel can't add the first venue (`createVenue` needs a business). Verified on an empty schema: migrate + seed:admin → admin + business, 0 venues. | `"sadece admin kullanıcısı… deploy edildiğinde… çalışır halde mi"` | ✅ fixed |
| B.35 | Username login + friendlier login page | Owner request | Login now takes **username + password** (`verifyCredentials` looks up by username; username editable in Güvenlik). Field pre-fills "admin" so existing flows/e2e are unaffected. Login page redesigned: **card + brand mark + title + show/hide password**. (Toggle's aria-label avoids "Şifre" and sits outside the label so `getByLabel` stays unambiguous.) | `"login olurken kullanıcı adı inputu neden yok"` + `"login sayfası daha user friendly olmalı"` | ✅ done |
| B.36 | "Group A" menu enrichments (owner asked what else businesses want, picked 3) | Owner request | **(1) Description + calories** — per-language product description (was in schema, now editable) + `Product.calories`; shown on the card. **(2) Diet/allergen badges** — `Product.dietary String[]` (Vegan/Vejetaryen/Glutensiz/Acı/Helal) via `src/lib/dietary.ts` (controlled vocab + localized labels); checkboxes in the form, badges on the card. **(3) Business contact** — `Business.hours/phone/instagram/mapUrl` in Settings → İşletme; a contact/social row in the public footer. One migration; each verified by e2e. | `"peki eklenmek istenirse neler eklenebilir"` → group A: `"Açıklama+kalori, Alerjen rozetleri, Saatler+iletişim"` | ✅ done |
| B.37 | "Group B" — QR / menu analytics | Owner picked group B; analytics built first | **PII-free menu-open analytics.** New `PageView` model (`venueSlug`, `locale`, `createdAt` only — no IP/UA/cookies) + migration. A `"use client"` `TrackView` beacon fires `POST /api/track` on the venue landing so the **public page stays static**; the Route Handler validates a known venue slug + supported locale and always returns `204`. New **Settings → Analitik** tab (`getAnalytics`) shows total / last-7 / last-30 opens, per-venue counts, a 14-day mini bar view, and the peak hour (Europe/Istanbul). Verified by an e2e (open a venue menu → the panel shows a non-zero count). | `"b"` → group B: `"QR / menü analitiği"` | ✅ done |
| B.38 | "Group B" — scheduled category visibility | Owner: also add timed visibility, then finalize | **Daily time-window per category.** `MenuCategory.visibleFrom/visibleTo` ("HH:MM", Europe/Istanbul) + migration; both null = always on, `from>to` = overnight (wraps midnight). Pure logic in `src/lib/schedule.ts` (`isOpenAt`, `nowHHMMInIstanbul`, `formatWindow`). **Guests** are filtered in `CategoryNavIsland` (already request-time via the `isAdmin` cookie read), so scheduled categories drop out of the **menu navigation** outside their window — the cached readers (`getVenueMenu`), sitemap, jsonLd and the category detail page are left untouched (window is a nav/merchandising control, not access control). **Admin** sees every category always, with a two-input window editor (validated both-or-neither) + a schedule badge. Window lives on the join (like `visible`/`sortOrder`), so — consistent with those — it is not in the Excel backup. Verified by an e2e (scheduled category hidden from a guest, still visible to admin). | `"zamanlı görünürlüğü de ekle ve öyle hizala"` | ✅ done |
